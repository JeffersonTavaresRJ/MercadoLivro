...
fun PutBookRequest.toBookModel(previousValue: BookModel): BookModel {
    return BookModel(id = previousValue.id,
...
fun BookModel.toResponse(): BookResponse {
    return BookResponse(id=this.id, name = this.name, price=this.price, customer = this.customer?.name, status=this.status);
}
...
fun <T> Page<T>.toPageResponse():PageResponse<T>{
    return PageResponse(this.content, this.totalElements, this.number,  this.totalPages);
}